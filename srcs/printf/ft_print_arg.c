...
int get_conversion_type(const char *s, int *index) {
    if (s[*index] && s[*index] == '%') {
        (*index)++;
        skip_space(s, index);
        if (is_special_space(s[*index])) return (CONV_IS_SPECIAL_SPACE);
        return (s[*index]);
    }
    return (0);
}

int print_known_conversion_type(const char *s, int *index, va_list *current_arg,
                                int conversion_type) {
...
int print_arg(const char *s, int *index, va_list *current_arg) {
    ...
    if (s[*index] && s[*index] == '%' && s[*index + 1]) {
        ...
        nb_char_written =
            print_known_conversion_type(s, index, current_arg, conversion_type);
